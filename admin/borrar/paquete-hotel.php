...
if(isset($_REQUEST['delId']) and $_REQUEST['delId']!=""){
    ...
    $delTour = $db->getAllRecords('paqHotel','*','AND id='.($_REQUEST['delId']).'','LIMIT 1');
    if (empty($delTour)) { 
	...
	header('location:/admin/nuevo/paquete-hotel?paqueteId='.($paqueteSel['id']).'');//exito
	exit;
}
?>