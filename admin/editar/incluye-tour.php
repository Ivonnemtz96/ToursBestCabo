...
if (isset($_REQUEST['editId']) and $_REQUEST['editId'] != "") {
    ...
    if (empty($incluyeSel)) { //SI NO EXISTE ES QUE NO HAY UN ID VALIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
        setcookie("msg", "tournoen", time() + 2, "/");
        header('location:/admin/nuevo/incluye-tour');
        exit;
    } else {
        $incluyeSel  =  $incluyeSel[0];
    }
} 
else {
    setcookie("msg", "tournoen", time() + 2, "/");
...
if (isset($_REQUEST['submit']) and $_REQUEST['submit'] != "") {
    ...
    if ($descripcion == "" && $descripcion_ingles = "") {
        ...
        $codigo = ($incluyeSel['imagen']); //SI NO SE SUBE LA FOTO LE DAMOS EL VALOR EXISTENTE QUE YA ESTÁ EN NUESTRA BASE DE DATOS
        ...
        if (!empty($_FILES['thumb']['tmp_name'])) {
            ...
            if ($_FILES['thumb']['type'] !== 'image/png') {
                ...
                header('location:/admin/editar/incluye-tour?editId=' . $_REQUEST['editId'] . '');
                exit;
            }
            ...
            if (isset($incluyeSel['imagen'])) {
                $archivo = '../../upload/incluye/' . (strftime("%Y/%m", strtotime(($incluyeSel['fr'])))) . '/' . ($incluyeSel['imagen']) . '.png';
            ...
            $ruta = '../../upload/incluye/' . (strftime("%Y/%m", strtotime(($incluyeSel['fr'])))) . '';
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Descripción</label>
                                                    <input name="descripcion_ingles" value="<?php echo ($incluyeSel['descripcion_ingles']); ?>" type="text" class="form-control">
                                                </div>
                                            </div>
                                            <div class="col-lg-4 col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-12 col-md-7">
                                                        <img class="responsive" src="/upload/incluye/<?php echo (strftime("%Y/%m", strtotime(($incluyeSel['fr'])))); ?>/<?php echo ($incluyeSel['imagen']) ?>.png" alt="">
                                                        <div>
