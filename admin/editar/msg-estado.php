...
if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	$msgStatus  =  $db->getAllRecords('msgStatus','*',' AND id="'.$_REQUEST['editId'].'"','LIMIT 1');
    
        if (empty($msgStatus)) { //SI NO EXISTE ES QUE NO HAY UN ID VALIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
            header('location:/admin/msg-estado?msg='.urlencode(encrypt("ups",$keyEN)).'');
            exit;
        } else {$msgStatus  =  $msgStatus [0];}
	...
	if($msg==""){
        ...
        if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	       ...
	       $update	=	$db->update('msgStatus',$data,array('id'=>($_REQUEST['editId'])));
            ...
            if($update){
                setcookie("msg","msgup",time() + 2, "/");
                header('location: /admin/msg-estado'); //Exito en el cmabio
                exit;}
                else{
                        ...
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Clave</label>
                                    <input name="msg" value="<?php echo ($msgStatus['msg']);?>" type="text" class="form-control">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <div class="form-group">
                                    ...
                                    <input name="descripcion" value="<?php echo ($msgStatus['descripcion']);?>" type="text" class="form-control">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Mensaje que verá el usuario</label>
                                    <input name="mensaje" value="<?php echo ($msgStatus['mensaje']);?>" type="text" class="form-control">
                                </div>
                            </div>
                            <div class="col-sm-2">
