...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
    ...
    if (($zona=="")and($noches=="")){
		...
		$hotelCount	=	$db->getQueryCount('paqHotel','id');
		if($hotelCount[0]['total']<1000){
			$data	=	array(
							'zona'=>$zona,
							'hotel'=>$hotel,
							'paquete'=>$paqSel['id'],
			...
			$insert	=	$db->insert('paqHotel',$data);
            
			if($insert){
                
                    //SUMAMOS +1 A LAS PROPIEDADES DE ESTE USUARIO
                    $Sum = (($paqSel['hotelCount'])+1);
				...
				header('location:/admin/nuevo/paquete-hotel?paqueteId='.($paqSel['id']).'');//exito
				exit;
			}else{
                        ...
                        <form method="post" enctype="multipart/form-data">
                            <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Noches</label>
                                            <input name="noches" class="form-control" type="number" required>
                                            <div class="invalid-feedback">
                                              Por favor ingresa esto por favor.
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                        <label>Hotel (Si no se selecciona el usuario podrá elegir)</label>
                                        <select name="hotel" class="form-control selectric">
                                            <option selected disabled>Selecciona uno para fijar</option>
                                            <?php
                                            $catData = $db->getAllRecords('hoteles','*',' ORDER BY nombre ASC');
                                            if (count($catData)>0){
                                                $y	=	'';
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $paquetesData = $db->getAllRecords('paqHotel','*','AND paquete='.$paqSel['id'].' LIMIT '.$paqSel['hotelCount'].'');
                                  if (count($paquetesData)>0){
                                      $y	=	'';
                                          foreach($paquetesData as $paquete){
                                              $zonaSel = $db->getAllRecords('zonas','*','AND id='.$paquete['zona'].'','LIMIT 1');
                                              $zonaSel = $zonaSel[0]['nombre'];
                                              
                                              $hotelSel = $db->getAllRecords('hoteles','*','AND id='.$paquete['hotel'].'','LIMIT 1');
                                              
                                              if (!$hotelSel) {
                                                  $hotelSel ="El usuario elige";
                                              } else { $hotelSel = $hotelSel[0]['nombre']; }
                                              ...
                                              <tr>
                                                    ...
                                                    <td><?php echo $paquete['noches']; ?></td>
                                                    <td><?php echo $hotelSel ?></td>
                                                    <td class="text-rigth">
                                                        <a href="/admin/borrar/paquete-hotel?delId=<?php echo $paquete['id']; ?>" onClick="return confirm('¿Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                    </td>
                                              </tr>
